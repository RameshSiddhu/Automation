...
public class driverReeport {
	...
	private static void DriverManagement() throws InterruptedException {
		...
		Thread.sleep(1000);
		WebElement show = driver.findElement(By.xpath("//*[@id=\"mCSB_1_container\"]/ul/li[3]/ul/li[3]/a/span"));
		show.click();
	}
	...
	private static void nameFilter() {
		...
		if (actualName.equalsIgnoreCase(fName) && actualName.equalsIgnoreCase(sName)) {
			System.out.println("filter with name is working properly");
		} else {
			System.out.println("filter with name is not working properly");
	...
	private static void DateFilter() {
		...
		if (rows.size() > 3) {
			...
			if (actualDate.equals(expectedDate)) {
				System.out.println("date filter is working properly");
			} else {
				driver.quit();
			}
		}
